...
namespace WeatherAppUI.Services
{
    ...
    {
        public static async Task<string> RunWeatherCheck(string cityString)
        {
            StringBuilder telegramMessage = new StringBuilder();

            //string apiKey = Environment.GetEnvironmentVariable("WEATHER_API_KEY");
            string apiKey = "EC2D6Z9XX2XJKM9MS49ASLAA4"; 

            // Debug message for API key
            Console.WriteLine($"API Key loaded: {(string.IsNullOrEmpty(apiKey) ? "NOT FOUND" : "[KEY FOUND]")}");

            if (string.IsNullOrEmpty(apiKey))
            {
                return "ERROR: Weather API key not found in environment variables. Check your .env file.";
            }

            string url = $"https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/{cityString}?key={apiKey}&lang=en&unitGroup=metric";

            // Debug message for URL (partially obfuscated for security)
            Console.WriteLine($"API URL: https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/{cityString}?key=[HIDDEN]&lang=en&unitGroup=metric");
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                Console.WriteLine($"⚠️ Weather API call wasn't successful. Status: {response.StatusCode}");
                Console.WriteLine($"Error details: {errorBody}");
                return $"WEATHER API ERROR: {response.StatusCode} - Please check console for details";
            }
            ...
            telegramMessage.AppendLine($"Forecast for today in {cityString}: ");
